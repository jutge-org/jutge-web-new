...
export type { ParseFailure, ParseResult, ParseSuccess, SmoothUIPackageMeta } from './smoothui-schema'
...
export const peopleData: Person[] = [
    ...
    },
    {
        ...
        stars: 5,
        content: 'The design system is incredibly well thought out. Every component feels intentional and polished.',
    },
    {
        name: 'Marcus Johnson',
        role: 'Lead Developer',
    ...
    },
    {
        ...
        stars: 4,
        content: "Our users love the smooth interactions. It's made our product feel premium and professional.",
    },
    {
        name: 'Mollie Hall',
        role: 'CTO',
        ...
        stars: 5,
        content: 'The performance is outstanding. Our bundle size stayed the same while getting beautiful animations.',
    },
    {
        name: 'Alec Whitten',
        role: 'UX Researcher',
        ...
        stars: 5,
        content: 'The accessibility features are top-notch. Every component follows WCAG guidelines perfectly.',
    },
    {
        name: 'Alisa Hester',
...
export const testimonialsData: Person[] = peopleData.filter((person) => person.stars && person.content)
...
export function getTeamMembers(count = 4, includeTestimonials = false): Person[] {
    if (includeTestimonials) {
        return peopleData.slice(0, count)
    }
    // Return people who don't have testimonials for team display
    return peopleData.filter((person) => !(person.stars && person.content)).slice(0, count)
}
...
export function getPeopleByRole(role: string): Person[] {
    return peopleData.filter((person) => person.role.toLowerCase().includes(role.toLowerCase()))
}
