...
function StarIcon({ filled, half }: { filled: boolean; half?: boolean }) {
    ...
    return (
        <svg
            aria-hidden="true"
            className={cn('h-3.5 w-3.5', filled ? 'text-amber-400' : 'text-muted-foreground/30')}
    ...
    return (
        <div aria-label={`${rating} out of 5 stars`} className="flex items-center gap-0.5">
            {Array.from({ length: 5 }, (_, i) => {
                ...
                return <StarIcon filled={isFilled} half={isHalf} key={`star-${title}-${i}`} />
            })}
...
export default function ProductCard({
    ...
    const handleAddToCart = () => {
        onAddToCart?.()
        // Skip the transient "Added" flash when a custom handler is provided
        // (e.g. opening a dialog instead of completing a purchase).
        if (!onAddToCart) {
            setIsAdded(true)
                ...
                <div className="relative aspect-square overflow-hidden rounded-xl bg-muted">
                    ...
                    {badge && (
                        <motion.span
                            ...
                            className={cn(
                                'absolute top-2.5 left-2.5 rounded-full px-2.5 py-1 font-semibold text-xs shadow-sm',
                    ...
                    <motion.button
                        ...
                        className={cn(
                            'absolute top-2.5 right-2.5 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-background/80 backdrop-blur-sm',
            ...
            <div className="flex flex-1 flex-col gap-2 p-4">
                ...
                <div className="mt-auto pt-2">
                    <SmoothButton
                        ...
                        className={cn('h-8 w-full gap-2', isAdded && 'bg-emerald-600 text-white hover:bg-emerald-600')}
                        disabled={isAdded}
