...
function getInitialSize(size: number | string | ResponsiveSize | undefined, defaultValue: number | string): string {
    if (!size) {
...
function getSizeForBreakpoint(size: ResponsiveSize, width: number): number | string | undefined {
...
function useResponsiveSize(size: number | string | ResponsiveSize | undefined, defaultValue: number | string): string {
    const [currentSize, setCurrentSize] = useState<string>(() => getInitialSize(size, defaultValue))
...
export default function AppleInvites({
    ...
    const explicitHeight = useResponsiveSize(cardHeight, calculateHeightFromWidth(responsiveWidth, aspectRatio))
    ...
    useEffect(() => {
        if (cardHeight === undefined) {
            setCalculatedHeight(calculateHeightFromWidth(responsiveWidth, aspectRatio))
        }
    }, [responsiveWidth, aspectRatio, cardHeight])

    const responsiveHeight = cardHeight === undefined ? calculatedHeight : explicitHeight
    ...
    const badgeFontSize = Math.max(MIN_BADGE_FONT_SIZE, Math.round(BASE_BADGE_FONT_SIZE * scaleFactor))
    const badgePaddingX = Math.max(MIN_BADGE_PADDING_X, Math.round(BASE_BADGE_PADDING_X * scaleFactor))
    ...
    const badgePaddingY = Math.max(
        MIN_BADGE_PADDING_Y,
        Math.round(BASE_BADGE_PADDING_Y * scaleFactor * BADGE_PADDING_Y_SCALE_FACTOR),
    )
    const badgeIconSize = Math.max(MIN_BADGE_ICON_SIZE, Math.round(BASE_BADGE_ICON_SIZE * scaleFactor))
    const titleFontSize = Math.max(MIN_TITLE_FONT_SIZE, Math.round(BASE_TITLE_FONT_SIZE * scaleFactor))
    const subtitleFontSize = Math.max(MIN_SUBTITLE_FONT_SIZE, Math.round(BASE_SUBTITLE_FONT_SIZE * scaleFactor))
    const locationFontSize = Math.max(MIN_LOCATION_FONT_SIZE, Math.round(BASE_LOCATION_FONT_SIZE * scaleFactor))
    const avatarSize = Math.max(MIN_AVATAR_SIZE, Math.round(BASE_AVATAR_SIZE * scaleFactor))
    const contentPadding = Math.max(MIN_CONTENT_PADDING, Math.round(BASE_CONTENT_PADDING * scaleFactor))
    const badgeTop = Math.max(MIN_BADGE_TOP, Math.round(BASE_BADGE_TOP * scaleFactor))
    const badgeLeft = Math.max(MIN_BADGE_LEFT, Math.round(BASE_BADGE_LEFT * scaleFactor))
    const badgeGap = Math.max(MIN_BADGE_GAP, Math.round(BASE_BADGE_GAP * scaleFactor))
    const avatarGap = Math.max(MIN_AVATAR_GAP, Math.round(BASE_AVATAR_GAP * scaleFactor))
    const avatarMarginBottom = Math.max(MIN_AVATAR_MARGIN_BOTTOM, Math.round(BASE_AVATAR_MARGIN_BOTTOM * scaleFactor))
    const titleMarginBottom = Math.max(MIN_TITLE_MARGIN_BOTTOM, Math.round(BASE_TITLE_MARGIN_BOTTOM * scaleFactor))
    ...
    const visibleEvents = [-1, 0, 1].map((offset) => events[wrap(0, events.length, activeIndex + offset)])
                ...
                >
                    <div className="relative h-full w-full overflow-hidden rounded-3xl bg-primary">
                        ...
                        >
                            ...
                            >
                                <CrownIcon aria-hidden size={badgeIconSize} />
                                {event.badge}
                            </span>
                        </div>
                        {/* Content */}
                        <div
                            className="absolute inset-x-0 bottom-0 z-[3] bg-gradient-to-t from-black/70 via-black/35 to-transparent text-white"
