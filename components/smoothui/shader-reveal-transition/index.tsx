...
void main(){
  ...
  if (uVariant < 0.5) {
    ...
    float x = uDirection < 0.0 ? 1.0 - uv.x : uv.x;
    float n = noise(vec2(x, uv.y) * vec2(12.0, 26.0) + vec2(uTime * 0.26, -uTime * 0.12));
...
const createController = (
  ...
  const tick = () => {
    ...
    gl.uniform1f(uDirection, direction);
  ...
  return {
    ...
    setDirection: (nextDirection: 1 | -1) => {
...
const ShaderRevealTransition = ({
  ...
  useEffect(() => {
    ...
    controller.setDirection(directionRef.current);
    setIsActive(true);
