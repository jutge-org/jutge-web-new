    ...
    const CardContent = (
        <div
            ...
            style={
                isNextVariant
                    ? ({
                          background: 'linear-gradient(110deg, oklch(0.65 0 0) 0.06%, oklch(0.54 0 0) 100%)',
                      } as React.CSSProperties)
                    : undefined
            }
        >
            {/* Illustration/Switchboard */}
            <div className="flex h-full w-full items-center justify-center" data-illustration="true">
                <div
            ...
            <div className="mt-4 flex flex-row items-center justify-start gap-2 text-left">
                <h3
                    ...
                    data-title="true"
                    style={isNextVariant ? ({ color: 'oklch(0.95 0 0)' } as React.CSSProperties) : undefined}
                >
                    {title}
                </h3>
            </div>
            ...
            <p
                className={cn(
                    ...
                    isNextVariant ? 'mt-1 max-w-[80%] text-base' : 'mt-1 text-foreground/70',
                )}
                data-subtitle="true"
                style={isNextVariant ? ({ color: 'oklch(0.9 0 0)' } as React.CSSProperties) : undefined}
            >
                {subtitle}
            </p>
        </div>
...
function LightBulb({ state, transitionDuration }: { state: LightState; transitionDuration: number }) {
    const transitionStyle = {
        transition: `opacity ${transitionDuration}ms ease-out, transform ${transitionDuration}ms ease-out`,
    }

    // Keep bulbs as pinpoints; glow uses blur only (no spread) so lights don't become fat capsules.
    const sizePx = state === 'high' ? 2 : 1.5

    return (
        <div
            className={cn('relative shrink-0 place-self-center rounded-full', state === 'off' && 'bg-foreground/40')}
            data-state={state}
            style={{
                ...transitionStyle,
                width: sizePx,
            ...
            {state === 'medium' && (
                <div
                    ...
                    style={{
                        ...
                        boxShadow: '0 0 2px color-mix(in oklch, var(--color-brand), transparent 45%)',
                        opacity: 0.85,
                    }}
                />
            )}
            ...
            {state === 'high' && (
                <div
                    ...
                    style={{
                        ...
                        backgroundColor: 'var(--color-brand)',
                        boxShadow:
                            '0 0 2px color-mix(in oklch, var(--color-brand), transparent 25%), 0 0 4px color-mix(in oklch, var(--color-brand), transparent 55%)',
                        opacity: 1,
                    }}
                />
            )}
